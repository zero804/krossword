    ...
    : QDialog(parent, flags), m_krossWord(krossWord)
{
    ...
    ui_convert_crossword_type.setupUi(this);
    setModal(true);

    ui_convert_crossword_type.toolBox->setItemText(0, i18n("Convert From: %1", krossWord->crosswordTypeInfo().name));
    ui_convert_crossword_type.typeInfoFromWidget->setElements(CrosswordTypeWidget::DefaultElements | CrosswordTypeWidget::ElementIcon);
    ui_convert_crossword_type.typeInfoToWidget->setElements();
    ui_convert_crossword_type.typeInfoFromWidget->setTypeInfo(krossWord->crosswordTypeInfo());
...
{
    QModelIndex current = ui_convert_crossword_type.crosswordType->model()->index(index, ui_convert_crossword_type.crosswordType->modelColumn());
    QModelIndex previous;

    if (m_previousConvertToTypeIndex != -1) {
        previous = ui_convert_crossword_type.crosswordType->model()->index(m_previousConvertToTypeIndex, ui_convert_crossword_type.crosswordType->modelColumn());
    }
    ...
    if (m_changedUserDefinedSettings) {
        if (static_cast<CrosswordType>(previous.data(Qt::UserRole + 1).toInt()) != UserDefinedCrossword) {
            return;
        } else if (KMessageBox::warningContinueCancel(this, i18n("Changing the crossword type will discard user defined crossword type settings."),
    ...
    ui_convert_crossword_type.toolBox->setItemText(1, i18n("Convert To: %1", m_convertTypeInfo.name));
    ui_convert_crossword_type.typeInfoToWidget->setTypeInfo(m_convertTypeInfo);

    // Update conversion info
    KrossWord::ConversionInfo conversionInfo = m_krossWord->convertToType(m_convertTypeInfo, true);
    ui_convert_crossword_type.conversionInfo->setText(m_krossWord->conversionInfoToString(conversionInfo));

    bool removedCells = !conversionInfo.cellsToRemove.isEmpty() || !conversionInfo.cluesToRemove.isEmpty();
    KColorScheme colorScheme(QPalette::Normal);
    QPalette palette = ui_convert_crossword_type.conversionInfo->palette();
    palette.setColor(QPalette::Normal, QPalette::WindowText, colorScheme.foreground(removedCells ? KColorScheme::NegativeText : KColorScheme::PositiveText).color());
    ...
    const CrosswordTypeInfo &typeInfo)
{
    ...
    // Update conversion info
    KrossWord::ConversionInfo conversionInfo = m_krossWord->convertToType(typeInfo, true);
    ui_convert_crossword_type.conversionInfo->setText(m_krossWord->conversionInfoToString(conversionInfo));
